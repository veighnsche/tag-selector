...
function findOptimizerTag({
  state,
  name,
  type,
}: FindOptimizerTagParams): keyof PromptTagsType | null {
...
export const tagsSlice = createSlice({
  ...
  reducers: {
    ...
    setTagsFromImageData: (
      state,
      action: PayloadAction<Partial<PromptTagsType>>
    ) => {
...
export const selectLocateTagByName =
  (state: RootState) =>
  (
    name: TagType['name']
...
export const selectLocateTag =
  (state: RootState) =>
  (id: TagType['id']): keyof PromptTagsType =>
    findTag(state.tags, id);

export const selectGetId =
  (state: RootState) =>
  (name: TagType['name']): TagType['id'] | undefined => {
    const { tags, negativeTags, tagPool } = state.tags;
    const tag = [...tags, ...negativeTags, ...tagPool].find(
      (tag) => tag.name === name
    );
    return tag?.id;
  };

export const selectHasOptimizerTag =
  (state: RootState) =>
  ({
    name,
    type,
    ...
    const { tags, negativeTags, tagPool } = state.tags;
    return [...tags, ...negativeTags, ...tagPool].some(
      (tag) => tag.name === name && tag.optimizer === type
    );
  };
