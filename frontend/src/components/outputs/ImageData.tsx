...
const StyledPaper = styled(Paper, {
  ...
  width: ${(props) =>
    props.open ? 30 /** todo: should be a 3:2 ratio */ + 'vw' : '0vw'};
  height: 100vh;
...
export const ImageData = ({ filename, open }: ImageDataProps) => {
  ...
  const { navigateFirst, navigateNext, navigatePrevious } =
    useModalNavigation();

  const negativeTags =
    data?.customData[ImageCustomData.PROMPT_TAGS].negativeTags;
  ...
  function replaceSelected() {
    if (data) {
      const selectedData = selected
        .filter((selected) =>
          propertyList.some((property) => property.property === selected)
        )
        .reduce((acc, curr) => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <ImageTagList
          tags={data?.customData.tags}
          promptTags={data?.customData.promptTags}
        />
        <Box p="1rem" mt="1rem">
